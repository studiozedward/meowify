...
(() => {
  ...
  const ensureBtn = () => { if (btn) return btn; btn = document.createElement('button'); btn.id = 'meowify-float'; btn.style.cssText = 'position:absolute;z-index:2147483647;padding:6px 10px;font:12px system-ui;color:#111;background:#fff;border:1px solid #ddd;border-radius:999px;box-shadow:0 4px 12px rgba(0,0,0,.15);cursor:pointer;display:none'; btn.onmousedown = e => e.preventDefault(); btn.onclick = () => openModal(btn.dataset.mode); document.body.appendChild(btn); return btn; };
  ...
  function openModal(mode) {
    ...
    const ov = document.createElement('div'); ov.id = 'meowify-overlay'; ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:2147483647;display:flex;align-items:center;justify-content:center;font-family:system-ui';
    const card = document.createElement('div'); card.style.cssText = 'background:#fff;color:#111;border-radius:16px;box-shadow:0 10px 30px rgba(0,0,0,.2);width:min(90vw,560px);max-height:80vh;display:flex;flex-direction:column;overflow:hidden';
    const head = document.createElement('div'); head.style.cssText = 'padding:14px 16px;border-bottom:1px solid #eee;font-weight:600'; head.textContent = mode === 'meow' ? 'Meowified text' : 'Unmeowified text';
    ...
    if (!ok) {
      ...
      const ta = document.createElement('textarea'); ta.value = res; ta.readOnly = true; ta.style.cssText = 'width:100%;height:160px;resize:vertical;padding:10px;border:1px solid #ddd;border-radius:8px;font:13px ui-monospace,monospace;color:#111;background:#fff';
      body.appendChild(ta);
    }
    ...
    const stats = document.createElement('div'); stats.style.cssText = 'padding:8px 16px;border-top:1px solid #eee;font:11px system-ui;color:#888;text-align:center';
    ...
    (async () => {
      try {
        ...
        if (data && data.messages && data.messages.length) {
          msgBox.style.cssText = 'padding:8px 16px;border-top:1px solid #eee;font:12px system-ui;color:#444';
    ...
    const foot = document.createElement('div'); foot.style.cssText = 'padding:12px 16px;border-top:1px solid #eee;display:flex;gap:8px;justify-content:flex-end';
    const close = document.createElement('button'); close.textContent = 'Close'; close.style.cssText = 'padding:8px 12px;border:1px solid #ddd;border-radius:8px;background:#f6f6f6;color:#111;cursor:pointer'; close.onclick = () => ov.remove(); foot.appendChild(close);
    if (ok) {
      const copy = document.createElement('button'); copy.textContent = 'Copy'; copy.style.cssText = 'padding:8px 12px;border:0;border-radius:8px;background:#111;color:#fff;cursor:pointer'; copy.onclick = async () => { await navigator.clipboard.writeText(res); copy.textContent = 'Copied!'; setTimeout(() => copy.textContent = 'Copy', 1200); }; foot.appendChild(copy);
      if (last.input || last.edit) { const rep = document.createElement('button'); rep.textContent = 'Replace'; rep.style.cssText = 'padding:8px 12px;border:0;border-radius:8px;background:#7c3aed;color:#fff;cursor:pointer'; rep.onclick = () => { doReplace(res); ov.remove(); }; foot.insertBefore(rep, copy); }
    }
